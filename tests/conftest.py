...
def _ensure_fields_shim():
    ...
    modules = [
        "skills.trend_fetcher",
        "skills.image_generator",
        "skills.wallet_manager",
    ]
                ...
                if isinstance(obj, type) and issubclass(obj, BaseModel):
                    ...
                    if mf:
                        for name, info in mf.items():
                            ...
                            default = (
                                info.get("default") if isinstance(info, dict) else None
                            )
...
def _patched_import(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    try:
        ...
        if name.startswith("skills") or (
            fromlist and any(str(x).startswith("skills") for x in fromlist)
        ):
            import importlib
            ...
            for attr in dir(mod):
                ...
                if isinstance(obj, type) and issubclass(obj, BaseModel):
                    ...
                    shim = {}
                    for fname, info in (
                        model_fields.items() if isinstance(model_fields, dict) else []
                    ):
